...
import torch
import cv2
from pysot.tracker.kcf import KalmanFilter
...
class SiamGATTracker(SiameseTracker):
    ...
    def track(self, img):
        ...
        #
        cls_score=cls_up[CRowUp, CColUp]
        #
        ...
        if cls_score<0.05:
            kf = KalmanFilter()
            m = np.array([cx, cy, width, height])   # 小车的初始化测量值，分别表示小车的x_center, y_center, w/h, h
            mean_ini, covariance_ini = kf.initiate(m) 
            mean_pre, covariance_pre = kf.predict(mean_ini, covariance_ini)
    ...
    def update_add(self,img,bbox):
        ...
        roi = torch.tensor([[c - bbox[2] * scale / 2, c - bbox[3] * scale / 2,
                             c + bbox[2] * scale / 2, c + bbox[3] * scale / 2]])
        zf_new=self.model.get_template_feature(z_crop, roi)
        self.model.update_add_zf(zf_new)