...
sub_sets=['ballet',   'bird1',  'car3',  'car9', 'cat2','freestyle',  'group3',     'nissan','person19',
...
def crop_video(video, d_set, crop_path, instanc_size):
    if video != 'list.txt' and video !='VOT2018-LT.json':
        # video_crop_base_path = join(crop_path, video)
        video_crop_base_path = join(crop_path, 'color')
        if not isdir(video_crop_base_path): makedirs(video_crop_base_path)
        gt_path = join(dataset_path, d_set, 'groundtruth.txt')
       
        images_path = join(dataset_path, d_set, 'color')
        ...
        for idx, gt_line in enumerate(groundtruth):
            ...
            cv2.imwrite(join(video_crop_base_path, '{:06d}.{:02d}.z.jpg'.format(int(idx_step), int(0))), z)
            cv2.imwrite(join(video_crop_base_path, '{:06d}.{:02d}.x.jpg'.format(int(idx_step), int(0))), x)
            idx_step=idx_step+1
